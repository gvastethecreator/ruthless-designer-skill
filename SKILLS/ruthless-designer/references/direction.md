...
Before making direction cards, record a justified vector:

```text
expression: quiet instrument <-> overt character
density: focused sequence <-> scan-heavy field
motion: instant/static <-> choreographed/direct
familiarity: established grammar <-> learned interaction
confidence: cautious/evidentiary <-> declarative/authoritative
emotional tone: exact words, not a slider
...
```text
killed default:
killed second reflex:
selected direction:
calibration vector + rationale:
signature move:
where it appears:
kill condition:
